...
```{r}
# # Classify subreddit as NSFW
# patterns = c("porn","gw","gonewild","tits","boob", "penis", "boner", "bbw",
#              "wifesharing", "nipple", )
# liwc_df_long %>%
#   mutate(is_nsfw = any(patterns %in% str_to_lower(subreddit)))
...
# a <- url_data %>%
#   read_html() %>%
#   html_nodes(xpath='//*[@id="listing-parent"]/div[1]') %>%
#   html_table()
# 
# 
# # url_data <- "https://www.reddit.com/r/ListOfSubreddits/wiki/nsfw"
# 
# a <- url_data %>%
#   read_html() %>% 
...
# pattern = c("subreddit")
# 
# substring="[Ss]ubreddit|[Ff]irst|[Rr]ecent|[Ll]ast|sinfopanelformoreoptions"
# 
# url_data <- "http://redditlist.com/all"
# selector_copy <- "#listing-parent > div.span4.listing.active-listing"
# 
# url_data %>%
#   read_html() %>% 
#   html_nodes(css = selector_copy) %>%
#   html_text()
# 
# xpath_copy <- '//*[@id="listing-parent"]/div[1]'
# url_data %>%
#   read_html() %>% 
#   html_nodes(xpath = xpath_copy) %>%
#   html_text()
# 
# nsfw_subs <- 
#   url_data %>%
...
# liwc_df_wide %>%
#   sample_n(100) %>%
#   ggplot(mapping = aes(x = posemo, y = swear)) +
#   geom_point() +
#   geom_text_repel(mapping = aes(label = subreddit)) + 
#   scale_x_log10() +
#   scale_y_log10()
# ```
# 
...
```{r}
# library(cluster)
# clusGap(liwc_df_wide %>% select(-subreddit), kmeans, 10, B = 100, verbose = interactive())
```

```{r}
# k.means.fit <- kmeans(liwc_df_wide %>% select(-subreddit), 4)
# 
# clusplot(liwc_df_wide %>% select(-subreddit), k.means.fit$cluster, main='2D representation of the Cluster solution', color=TRUE, shade=TRUE, labels=2, lines=0)
# 
